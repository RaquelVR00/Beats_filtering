...
Resultados = "Resultados_promedio2.txt"
...
for (j in 1:length(change)){
  ...
  for(i in 1:c){
    ...
    l <- NULL
    for(l in 20:(length(vector_RR)-3)){
    ...
    for(l in 20:(length(vector_RR)-3)){
      suma=0
      beat_evaluated = vector_RR[l+1]
      #Si se excede la longitud del vector que contiene los RR normales else
      if(length(vector_solo_Off)>l){
        #Si el RR normal en la posicion l es un RR normal futuro respecto a 
        #RR a evaluar dirgirse a else
        if(vector_solo_Off[l]<(l+1)){
          
          #Calculamos el promedio
          for(r in l:l-(numero_x-1)){
            suma=vector_RR_2[r]+suma
          }
          promedio=suma/numero_x
          
        }else{
          #Buscamos el RR normal anterior al RR a evaluar
          for( d in l:1){
            if(vector_solo_Off[d]<=l){
              #Al encontrar la posicion del RR normal anterior al RR a evaluar
              #salimos del bucle
              break
            }
          }
          #Calculamos el promedio 
          for(o in d:(d-(numero_x-1))){
            suma=vector_RR_2[o]+suma
          }
          promedio=suma/numero_x
        }
      }else{
        #Partimos del ultimo RR normal
        #Si el ultimo RR normal es un RR futuro al RR a evaluar else
        if(vector_solo_Off[length(vector_solo_Off)]<(l+1)){
          #Si no lo es calculamos el promedio de los ultimos numero_x RR normales
          for(z in length(vector_solo_Off):(length(vector_solo_Off)-(numero_x-1))){
            suma=vector_RR_2[z]+suma
          }
          promedio=suma/numero_x
        }else{
  ...
  #numero_escogido=paste("Numero_promedio_",numeros[s],sep="")
