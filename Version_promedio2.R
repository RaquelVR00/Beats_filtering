      ...
      for(l in 1:(length(vector_RR)-3)){
        ...
        if(length(vector_solo_Off)>l){
          ...
          if(vector_solo_Off[length(vector_solo_Off)]<(l+1)){
            for(r in length(vector_solo_Off):(length(vector_solo_Off)-numero_x)){
              suma=vector_RR_2[r]+suma
              cont=cont+1
              div=div+1
