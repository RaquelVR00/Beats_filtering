...
for( j in 1:c){
  ...
  suma=0
  vector_flags_berntson<-vector_flags #copia del vector que contiene las flags 
  ...
  for(l in 1:(length(vector_RR)-3)){
    ...
    if(l>=anterior){
      for(r in l:1){ 
        if(vector_flags_berntson[r]=="0"){
          suma=vector_RR[r]+suma
          contador_RR=contador_RR+1
        }
